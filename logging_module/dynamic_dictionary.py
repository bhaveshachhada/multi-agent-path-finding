...
def config_initial(config_file_path):
    ...
    try:
        with open(config_file_path, 'w') as config:
            json.dump(log_setting, config, indent=4)
...
def update_config_dictionary(logger_name, base_path, config_file_path, handler_type, logging_level, host,
    ...
    @param host:
    @param port:
    @param rotating_maxyBytes:
    @param rotating_backupCount:
    @param timed_rotating_when:
    @param timed_rotating_interval:
    @param timed_rotating_backupCount:
    """
    ...
    if handler_type == constants.CONST_STREAM_HANDLER:
        options_dictionary["class"] = 'logging.StreamHandler'
    ...
    try:
        with open(config_file_path, 'w') as config:
            json.dump(config_dictionary, config, indent=4)
        # lock.release()
    except ValueError as e:
        print("Error Occurred : {0}".format(str(e)))
        # lock.release()
