...
root_log_base_path = os.path.join("logs", str(datetime.datetime.now().strftime("%Y")),
...
def create_and_load_config_file():
    ...
    if not os.path.exists(root_log_base_path):
        os.makedirs(root_log_base_path)
...
def create_logger(logger_name, handler_type, logging_level="DEBUG", host=constants.CONST_DEFAULT_HOST_IP,
                  port=constants.CONST_DEFAULT_PORT, rotating_maxy_bytes=1024000, rotating_backup_count=100,
                  timed_rotating_when='D', timed_rotating_interval=1, timed_rotating_backup_count=2):
    """
    ...
    :param rotating_maxy_bytes: For ROTATING_FILE_HANDLER's maxBytes arg [optional, Default: 102400 Bytes]
    ...
    :param timed_rotating_backup_count: For TIMED_ROTATING_FILE_HANDLER's backupCount arg [optional, Default: 2]
    ...
    for handler_item in handler_type:
        ...
        if result:
            ...
            handler = get_handler(logger_name, handler_item, host, port, rotating_maxy_bytes, rotating_backup_count,
                                  timed_rotating_when, timed_rotating_interval, timed_rotating_backup_count)
            ...
            dynamic_dictionary.update_config_dictionary(logger_name, base_path, config_file_path, handler_item,
                                                        logging_level, host, port, rotating_maxy_bytes,
                                                        rotating_backup_count, timed_rotating_when,
                                                        timed_rotating_interval, timed_rotating_backup_count)
...
def get_mapping_list():
    ...
    if not os.path.isfile(mapping_list_path):
        ...
        with open(mapping_list_path, 'w') as mapping:
            json.dump({}, mapping)
...
def update_mapping_list():
    ...
    try:
        with open(mapping_list_path, 'w') as json_file:
            json.dump(logger_mapping_dictionary, json_file, indent=4)
        # lock.release()
    except ValueError as e:
        print("Error Occurred : {0}".format(str(e)))
        # lock.release()
...
def get_handler(logger_name, handler_type, host, port, rotating_maxy_bytes, rotating_backup_count, timed_rotating_when,
                timed_rotating_interval, timed_rotating_backup_count):
    """
    ...
    @param handler_type: 
    @param logger_name: 
    @param timed_rotating_backup_count: 
    @param timed_rotating_interval: 
    @param timed_rotating_when: 
    @param rotating_backup_count: 
    @param rotating_maxy_bytes: 
    """
    ...
    if handler_type == constants.CONST_STREAM_HANDLER:
        ...
        rotating_file_handler = logging.handlers.RotatingFileHandler(log_path, 'a', maxBytes=rotating_maxy_bytes,
...
def setup_logger(logger_name, level="INFO", use_stream_handler=False):
    if not use_stream_handler:
        my_logger = create_logger(logger_name=logger_name,
                                  handler_type=[constants.CONST_SOCKET_HANDLER, constants.CONST_ROTATING_FILE_HANDLER],
                                  logging_level=level,
                                  host='localhost',
                                  port=9020)

    else:
        my_logger = create_logger(logger_name, [constants.CONST_STREAM_HANDLER], logging_level=level)

    # print ("My Logger is: ", logger_name, use_stream_handler)
    return my_logger
...
def config_log():
    try:
        if not constants.IS_LOG_SETUP_IMPORTED:
            ...
            print("logger configration imported")
    except:
        print("config_log: exception:" + str(traceback.print_exc()))
